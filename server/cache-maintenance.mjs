import {deduplicateReleaseAssets} from './release-dedup.mjs';
...
export async function cleanReleaseCache(root,protectedVersions,{now=Date.now(),maxAge=3600000}={}){
 const keep=new Set([...protectedVersions,...(globalThis[Symbol.for('harness.backendReleaseLeases')]?.keys()||[])]),releases=[];let removed=0,removedStaging=0;
 let entries;try{entries=await fs.readdir(root,{withFileTypes:true});}catch(e){if(e.code==='ENOENT')return {removed};throw e;}
 for(const e of entries){const lease=e.name.match(/^\.backend-lease-(\d+)\.json$/);if(!e.isFile()||!lease)continue;const file=path.join(root,e.name);try{process.kill(Number(lease[1]),0);}catch(error){if(error.code==='ESRCH'){await fs.rm(file,{force:true});continue;}if(error.code!=='EPERM')throw error;}
  let data;try{data=JSON.parse(await fs.readFile(file,'utf8'));}catch(error){if(error.code==='ENOENT')continue;throw Error('Cannot read live backend lease; cleanup skipped');}
  if(!Array.isArray(data.versions)||data.versions.some(v=>!versionPattern.test(v)))throw Error('Invalid live backend lease; cleanup skipped');for(const version of data.versions)keep.add(version);
 }
 ...
 const shared=await deduplicateReleaseAssets(root,releases.filter(r=>keep.has(r.id)||now-r.time<maxAge).map(r=>r.id));
 return {removed,removedStaging,retained:releases.length-removed,...shared};
}
