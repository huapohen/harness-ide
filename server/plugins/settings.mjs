...
export default {id:'settings',requires:['routes'],async activate(ctx){
 ...
 ctx.effect(ctx.get('routes').register('settings/layout/write',q=>{
  ...
  const task=layoutPending.then(async()=>{const current=await readLayout(),temp=layoutFile+'.'+randomUUID()+'.tmp';const value={...current,...(q.topTabAlignment===undefined?{}:{topTabAlignment:q.topTabAlignment}),...(q.autoSave===undefined?{}:{autoSave:q.autoSave}),...(secondaryWidth===undefined?{}:{secondaryWidth}),...(visibility===undefined?{}:{visibility}),...(order===undefined?{}:{activityOrder:order}),...(font===undefined?{}:{font})};try{await fs.writeFile(temp,JSON.stringify(value,null,2)+'\n',{mode:0o600});await fs.rename(temp,layoutFile);}finally{await fs.rm(temp,{force:true});}return value;});layoutPending=task.catch(()=>{});return task;
 }));
