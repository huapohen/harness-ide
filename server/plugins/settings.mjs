...
export default {id:'settings',requires:['routes'],async activate(ctx){
 ...
 ctx.effect(ctx.get('routes').register('settings/layout/write',q=>{
  ...
  const preferences=q.preferences;if(preferences!==undefined&&(!preferences||typeof preferences!=='object'||Array.isArray(preferences)||Object.entries(preferences).some(([k,v])=>!['editor-word-wrap','ide-zoom','harness-theme','harness-backgrounds-one-dark-pro','harness-backgrounds-light'].includes(k)||(v!==null&&(typeof v!=='string'||v.length>2048)))))throw Error('Invalid preferences');
