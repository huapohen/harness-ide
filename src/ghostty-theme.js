...
export function installThemeAdapter(term,initial){
 const renderer=term.renderer,render=renderer.render.bind(renderer);let foreground=new Map(),background=new Map(),dirty=false,lastFocused,cursorPositioned=false;
 ...
 const wrap= (target,method)=>new Proxy(target,{get(obj,prop){if(prop==='getCursor'&&method==='getLine')return ()=>{const cursor=obj.getCursor();if(cursor.x>0||cursor.y>0)cursorPositioned=true;return {...cursor,visible:cursor.visible&&cursorPositioned&&(typeof document==='undefined'||term.element.contains(document.activeElement))};};if(prop===method)return i=>mapLine(obj[method](i));const value=Reflect.get(obj,prop);return typeof value==='function'?value.bind(obj):value;}});
