...
export async function restoreFiles(wb,session){
 for(const saved of session.tabs){
  ...
  }catch(error){logMessage('Cannot restore '+saved.title+': '+error.message);}
 }
...
export async function restoreLastSession(ctx){
 ...
 if(info.root!==session.workspace.root||(info.host||null)!==(session.workspace.host||null))await workspace.connect(session.workspace.root,session.workspace.host,{restoring:true});
 await restoreFiles(ctx.get('workbench'),session);return true;
}
export async function startFileSessions(kernel){
 ...
 const flush=()=>{clearTimeout(timer);const value=captureSession(wb,kernel.get('workspace').info()),text=JSON.stringify(value);const task=queue.catch(()=>{}).then(async()=>{if(text!==last){await api('settings/session/write',{session:value});last=text;}});queue=task;return task;};
