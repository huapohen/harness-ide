...
export function installTabActivation(node,activate,{drag=()=>{},drop=()=>{},cancel=()=>{}}={}){
 ...
 const stamp=e=>({time:e.timeStamp,x:e.clientX,y:e.clientY,id:e.pointerId,buttons:e.buttons});
 ...
 node.onpointerdown=e=>{if(e.button!==0||control(e)){start=null;moving=false;return;}e.preventDefault?.();moving=false;handled=stamp(e);const reversed=orphanUp?.id===e.pointerId&&same(orphanUp,e,20);orphanUp=null;start=reversed?null:stamp(e);activate(e);};
 node.onpointermove=e=>{if(start?.buttons&1&&!(e.buttons&1)){start=null;moving=false;cancel();return;}if(start&&start.id===e.pointerId&&(moving||Math.hypot(e.clientX-start.x,e.clientY-start.y)>6)){moving=true;node.setPointerCapture?.(e.pointerId);drag(e);}};
