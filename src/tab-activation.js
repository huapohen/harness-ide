...
export function installTabActivation(node,activate,{drag=()=>{},drop=()=>{},cancel=()=>{}}={}){
 ...
 node.onpointerup=e=>{const tap=start;start=null;if(node.hasPointerCapture?.(e.pointerId))node.releasePointerCapture(e.pointerId);if(!tap)return;handled=true;if(moving){moving=false;drop(e);cancel();return;}if(tap.id===e.pointerId&&e.button===0&&!control(e))activate(e);};
