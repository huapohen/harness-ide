...
export default {id:'documents',requires:['api','workbench'],async activate(ctx){
 ...
 async function openFile(path,{external=false,fresh=false,duplicate=false,temporary=false,previewGroup='primary'}={}){
  ...
  let saved=new TextDecoder().decode(bytes);const source=el('textarea','source-editor');source.value=saved;source.setSelectionRange(0,0);source.spellcheck=false;source.setAttribute('aria-label','文件内容');tab.editor=source;tab.focus=()=>{if(source.isConnected)source.focus();};let editor=sourceEditor(source,/(^|\/)\.ssh\/config$/.test(path)?'sshconfig':ext);const disposeLinks=markdownInteractions({element,source,tab,api,open,onError:logMessage});tab.showLinkHover=()=>disposeLinks.showHover();tab.dispose=()=>{disposeLinks();disposePreview();editor.dispose();};const preview=el('div','preview');let mode=['md','markdown'].includes(ext)?'source':viewers.has(ext)?'preview':'source';
  const disposePreview=()=>{preview.disposePreview?.();preview.disposePreview=undefined;preview.disposePreviewScrollbar?.();preview.disposePreviewScrollbar=undefined;preview.disposePreviewImages?.();preview.disposePreviewImages=undefined;preview.replaceChildren();};
  const show=()=>{disposeLinks.hide();disposePreview();content.replaceChildren();if(mode==='source'){content.append(editor.element);editor.refresh();}else{preview.replaceChildren();viewers.get(ext)(preview,source.value,{path:tab.path,external:tab.external});content.append(preview);}};
