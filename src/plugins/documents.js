...
export default {id:'documents',requires:['api','workbench'],activate(ctx){
 ...
 const modeActions=el('div','document-tab-actions');wb.$('.tab-actions').prepend(modeActions);
 ctx.on('tabs.changed',()=>{const b=wb.active()?.modeButton;modeActions.replaceChildren(...(b?[b]:[]));});ctx.effect(()=>modeActions.remove());
 ...
 async function open(path,{external=false,fresh=false,duplicate=false}={}){
  ...
  let saved=new TextDecoder().decode(bytes);const source=el('textarea','source-editor');source.value=saved;source.setSelectionRange(0,0);source.spellcheck=false;source.setAttribute('aria-label','文件内容');tab.editor=source;tab.focus=()=>{if(source.isConnected)source.focus();};let editor=sourceEditor(source,/(^|\/)\.ssh\/config$/.test(path)?'sshconfig':ext);tab.dispose=()=>editor.dispose();const preview=el('div','preview');let mode=viewers.has(ext)?'preview':'source';
  const show=()=>{content.replaceChildren();if(mode==='source'){content.append(editor.element);editor.refresh();}else{preview.replaceChildren();viewers.get(ext)(preview,source.value);content.append(preview);}modeButton.className='document-icon codicon-'+(mode==='source'?'open-preview':'code');modeButton.title=mode==='source'?'切换到预览':'切换到源码';modeButton.setAttribute('aria-label',modeButton.title);};
  ...
  const modeButton=button('','切换源码 / 预览',toggleMode,'document-icon codicon-code'),typeLabel=el('span','document-type',ext.toUpperCase()+' DOCUMENT');tab.modeButton=modeButton;modeButton.hidden=!viewers.has(ext);if(ext==='md')tab.togglePreview=toggleMode;
