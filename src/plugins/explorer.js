...
export default {id:'explorer',requires:['api','workbench'],async activate(ctx){
 ...
 ctx.effect(dropTarget(()=>wb.$('.sidebar').dataset.panel==='explorer'?wb.$('#sidebar-body'):null,()=>({root:info.root,host:info.host}),async dir=>{rootExpanded=true;let p=dir;while(p!=='.'){expanded.add(p);p=parent(p);}await refresh();}));
