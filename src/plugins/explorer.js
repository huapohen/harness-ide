...
export default {id:'explorer',requires:['api','workbench'],async activate(ctx){
 ...
 const finish=async commit=>{if(done||input.disabled)return;const name=input.value.trim();if(!commit||name===old){done=true;label.textContent=old;if(!row.querySelector('input'))row.focus();return;}if(!name||name==='.'||name==='..'||/[\\/]/.test(name)){input.setCustomValidity('请输入有效名称');input.reportValidity();return;}input.disabled=true;
