...
export default {id:'explorer',requires:['api','workbench'],async activate(ctx){
 ...
 const finish=async commit=>{if(done)return;const name=input.value.trim();if(!commit||name===old){done=true;label.textContent=old;if(!row.querySelector('input'))row.focus();return;}if(!name||name==='.'||name==='..'||/[\\/]/.test(name)){input.setCustomValidity('请输入有效名称');input.reportValidity();return;}input.disabled=true;
 ...
 input.onclick=input.ondblclick=e=>e.stopPropagation();input.onkeydown=e=>{e.stopPropagation();if(e.key==='Enter'){e.preventDefault();finish(true);}if(e.key==='Escape'){e.preventDefault();finish(false);}};requestAnimationFrame(()=>{if(!input.isConnected)return;input.focus();const dot=old.lastIndexOf('.');input.setSelectionRange(0,row.classList.contains('tree-folder')||dot<=0?old.length:dot);});
 }
 ...
 async function tree(container,rel='.',depth=0){
  ...
  for(const entry of entries){
   const p=join(rel,entry.name),row=el('div','file-row');row.tabIndex=0;row.title=p;
   ...
   const toggle=async()=>{if(expanded.has(p)){expanded.delete(p);children.replaceChildren();}else{expanded.add(p);try{await tree(children,p,depth+1);}catch(e){expanded.delete(p);throw e;}}drawArrow();await persistTree();};
