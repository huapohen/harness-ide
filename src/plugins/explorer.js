...
export default {id:'explorer',requires:['api','workbench'],async activate(ctx){
 ...
 ctx.effect(wb.command('files.revealFocusedFinder','文件 · 在 Finder 中显示当前项目',()=>{const focused=document.activeElement;if(focused?.closest('.secondary-content'))return wb.run('rightExplorer.revealFinder');if(focused?.closest('.sidebar[data-panel="explorer"]'))return wb.run('files.revealFinder',{path:selected});const tab=wb.current();if(tab?.path&&tab.kind!=='terminal')return wb.run('files.revealFinder',tab);}));
