...
export default {id:'workbench',requires:['api'],async activate(ctx){
 ...
 applyZoom();
 const commands=new Map(),tabs=[],panels=new Map(),multiTabs=new Set();let hiddenPanels;try{hiddenPanels=new Set(JSON.parse(localStorage.getItem('hidden-panels')||'[]'));}catch{hiddenPanels=new Set();}let active=null,docked=null,lastDock=null,dockMaximized=false,sidebarId='explorer',closing=false;
 ...
 async function close(t=current()){
  if(!t||closing)return false;closing=true;
 ...
 async function closeAll(targets=tabs){
  ...
  try{
   const snapshot=[...targets].filter(t=>tabs.includes(t));
 ...
 const api={snapshotLayout:()=>{const encode=n=>n.children?{direction:n.direction,ratio:n.ratio,children:n.children.map(encode)}:{id:n.id};return {active:active?.id,docked:docked?.id,groups:[...new Set(tabs.map(t=>t.group).filter(Boolean))].map(g=>encode(g.tree)),sidebarId,sidebarHidden:$('.sidebar').classList.contains('collapsed'),sidebarWidth:$('.sidebar').style.width,dockHeight:$('.dock').style.height,dockMaximized};},restoreLayout:async state=>{const decode=n=>n.children?{direction:n.direction,ratio:n.ratio,children:n.children.map(decode).filter(Boolean)}:tabs.find(t=>t.id===n.id);for(const tree of state.groups||[]){const group={tree:decode(tree),element:el('div','terminal-group')};for(const t of leaves(group.tree))t.group=group;drawGroup(group);}active=tabs.find(t=>t.id===state.active)||tabs[0]||null;docked=tabs.find(t=>t.id===state.docked)||null;dockMaximized=state.dockMaximized;$('.dock').style.height=state.dockHeight;$('.sidebar').style.width=state.sidebarWidth;await api.showPanel(state.sidebarId||'explorer');$('.sidebar').classList.toggle('collapsed',state.sidebarHidden);render();active?.focus?.();},rename,tabs,commands,root,$,render,remove,select,current,split,active:()=>active,close,closeAll,
  ...
  open(tab){const existing=tabs.find(t=>t.id===tab.id);if(existing){select(existing);return existing;}tabs.push(tab);$('.editor-area').append(tab.element);select(tab);return tab;},
  panel(id,label,icon,renderPanel){const b=button(icon,label,()=>{visibility.sidebar=true;$('.sidebar').classList.remove('collapsed');applyVisibility();layoutAPI('settings/layout/write',{visibility:{...visibility}}).catch(logMessage);return api.showPanel(id);},'activity-button');const icons={explorer:'files',search:'search',git:'source-control',remote:'remote-explorer',settings:'settings-gear','plugin-manager':'extensions',plugins:'extensions'};if(icons[id]){b.textContent='';b.classList.add('codicon','codicon-'+icons[id]);}b.hidden=hiddenPanels.has(id);b.dataset.panelId=id;b.draggable=true;
   b.ondragstart=e=>{e.dataTransfer.setData('application/x-harness-activity',id);e.dataTransfer.effectAllowed='move';b.classList.add('activity-dragging');};
   ...
   b.ondragover=e=>{if(!Array.from(e.dataTransfer.types).includes('application/x-harness-activity'))return;e.preventDefault();e.dataTransfer.dropEffect='move';const after=e.clientY>b.getBoundingClientRect().top+b.getBoundingClientRect().height/2;b.classList.toggle('activity-drop-before',!after);b.classList.toggle('activity-drop-after',after);};
   b.ondragleave=()=>b.classList.remove('activity-drop-before','activity-drop-after');
   b.ondrop=e=>{const sourceId=e.dataTransfer.getData('application/x-harness-activity'),source=[...$('.activity').children].find(item=>item.dataset.panelId===sourceId);if(!source)return;e.preventDefault();e.stopPropagation();const after=e.clientY>b.getBoundingClientRect().top+b.getBoundingClientRect().height/2;if(source!==b)$('.activity').insertBefore(source,after?b.nextSibling:b);activityOrder=[...$('.activity').children].map(item=>item.dataset.panelId);localStorage.setItem('activity-order',JSON.stringify(activityOrder));persistOrder();for(const item of $('.activity').children)item.classList.remove('activity-drop-before','activity-drop-after');};
  ...
  async showPanel(id){const p=panels.get(id);if(!p)return;sidebarId=id;$('.sidebar').dataset.panel=id;$('#explorer-actions').hidden=id!=='explorer';$('.sidebar-heading span').textContent=id==='search'?p.label:p.label.toUpperCase();for(const [key,item]of panels)item.b.classList.toggle('selected',key===id);$('#sidebar-body').oncontextmenu=null;$('#sidebar-body').replaceChildren();await p.renderPanel($('#sidebar-body'));},
 ...
 window.harnessRequestQuit=async()=>{try{return await closeAll();}catch(e){logMessage(e);return false;}};ctx.effect(()=>delete window.harnessRequestQuit);
