...
export default {id:'workbench',requires:['api'],async activate(ctx){
 ...
 function layoutTopTabs(){const lane=$('.top-tabs'),nodes=[...lane.children];for(const item of topTabLayout(nodes.map(node=>({node,x:tabs.find(t=>t.id===node.dataset.tabId)?.topX||0,width:node.getBoundingClientRect().width})),lane.clientWidth)){item.node.style.left=item.left+'px';}}
