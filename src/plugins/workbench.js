...
export default {id:'workbench',requires:['api'],async activate(ctx){
 ...
 let visibility={activity:true,sidebar:true,status:true,titlebar:true,secondary:false};const applyVisibility=()=>{ctx.emit('sidebar.visibility',{id:sidebarId,visible:visibility.sidebar});for(const [key,value]of Object.entries(visibility))root.classList.toggle('view-hide-'+key,!value);};
 let topTabAlignment='free';const layoutAPI=ctx.get('api');let saveOrder=Promise.resolve();
 try{const saved=await layoutAPI('settings/layout/read');applyFonts(saved.font);if(Array.isArray(saved.hiddenPanels)){hiddenPanels=new Set(saved.hiddenPanels);localStorage.setItem('hidden-panels',JSON.stringify([...hiddenPanels]));}else if(hiddenPanels.size)await layoutAPI('settings/layout/write',{hiddenPanels:[...hiddenPanels]});if(['free','left','right','center'].includes(saved.topTabAlignment))topTabAlignment=saved.topTabAlignment;if(saved.secondaryWidth)$('.secondary-sidebar').style.width=saved.secondaryWidth+'px';Object.assign(visibility,saved.visibility);applyVisibility();if(Array.isArray(saved.activityOrder)){activityOrder=saved.activityOrder;localStorage.setItem('activity-order',JSON.stringify(activityOrder));}else if(activityOrder.length)await layoutAPI('settings/layout/write',{activityOrder});}catch(e){logMessage('Unable to load sidebar order: '+e.message);}
 ...
 function render(preserveTabs=false){
  ...
  for(const t of tabs){
   ...
   installTabActivation(b,e=>{if(e.shiftKey&&!e.altKey){const chosen=selectRange(tabs,tabSelectionAnchor||active,t,multiTabs,e.metaKey);multiTabs.clear();for(const x of chosen)multiTabs.add(x);render();}else if(e.metaKey){tabSelectionAnchor=t;if(multiTabs.has(t))multiTabs.delete(t);else{if(!multiTabs.size&&active)multiTabs.add(active);multiTabs.add(t);}render();}else{tabSelectionAnchor=t;multiTabs.clear();multiTabs.add(t);select(t);}if(e.altKey){if(t.kind==='terminal')t.copyCwd?.(!e.shiftKey).catch(logMessage);else if(t.path)api.run('files.copyPath',[t],!e.shiftKey).catch(logMessage);}},{drag:e=>{const top=e.clientY<root.getBoundingClientRect().top+Math.max(28,$('.titlebar').getBoundingClientRect().height);root.classList.toggle('tab-snap-top',top);root.classList.toggle('tab-snap-bottom',!top);},drop:e=>{const top=root.classList.contains('tab-snap-top');const target=document.elementFromPoint(e.clientX,e.clientY)?.closest('.tab');t.topBar=top;if(top){const box=$('.top-tabs').getBoundingClientRect();t.topX=Math.max(0,e.clientX-(box.width?box.left:85)+$('.top-tabs').scrollLeft-b.getBoundingClientRect().width/2);}t.temporary=false;if(target&&target.dataset.tabId!==t.id){const other=tabs.find(x=>x.id===target.dataset.tabId);if(other){tabs.splice(tabs.indexOf(t),1);tabs.splice(tabs.indexOf(other),0,t);}}render();},cancel:()=>root.classList.remove('tab-snap-top','tab-snap-bottom')});b.onkeydown=e=>{if(e.key==='Enter')select(t);};b.oncontextmenu=e=>{e.preventDefault();e.stopPropagation();if(!multiTabs.has(t)){multiTabs.clear();multiTabs.add(t);}const selectedTabs=tabs.filter(x=>multiTabs.has(x));const index=tabs.indexOf(t),others=tabs.filter(x=>!multiTabs.has(x)&&!x.pinned),right=tabs.slice(index+1).filter(x=>!x.pinned);const items=[{label:t.pinned?'Unpin':'Pin',run:()=>{const value=!t.pinned;for(const x of selectedTabs)x.pinned=value;tabs.sort((a,b)=>Number(!!b.pinned)-Number(!!a.pinned));render();}},null,{label:selectedTabs.length>1?'Close Selected':'Close',disabled:selectedTabs.every(t=>t.pinned),run:()=>selectedTabs.length>1?closeAll(selectedTabs):close(t)},{label:'Close Others',disabled:!others.length,run:()=>closeAll(others)},{label:'Close to the Right',disabled:!right.length,run:()=>closeAll(right)},{label:'Close All',disabled:tabs.every(t=>t.pinned),run:()=>closeAll()}];if(t.canPreview?.())items.push(null,{label:'Edit Mode',checked:t.getMode()==='source',run:()=>{select(t);t.showMode('source');}},{label:'Preview Mode',checked:t.getMode()==='preview',run:()=>{select(t);t.showMode('preview');}});if(t.path)items.push(null,{label:'Copy Path',run:()=>api.run('files.copyPath',selectedTabs)},{label:'Copy Relative Path',run:()=>api.run('files.copyPath',selectedTabs,true)},null,{label:'Reveal in Finder',run:()=>api.run('files.revealFinder',t)},{label:'Reveal in Explorer View',run:()=>api.run('explorer.revealFile',t)},{label:'Split Right',run:()=>api.run('file.splitRight',t)});if(t.kind!=='terminal'&&t.save)items.push(null,{label:'Save',run:()=>t.save()});if(t.kind==='terminal')items.push(null,{label:'Copy Path',run:()=>t.copyCwd()},{label:'Copy Relative Path',run:()=>t.copyCwd(true)},{label:'Rename',disabled:true,run:()=>rename(t)},null,{label:'Split Right',run:()=>{select(t);return api.run('terminal.splitVertical');}},{label:'Split Down',run:()=>{select(t);return api.run('terminal.splitHorizontal');}});menuAt(e.clientX,e.clientY,items);};
   $(t.topBar?'.top-tabs':'.tabs').append(b);
  }
  }else{for(const [i,t]of tabs.entries()){const b=[...root.querySelectorAll('.tab')].find(b=>b.dataset.tabId===t.id);b?.classList.toggle('active',active===t);b?.classList.toggle('docked',docked===t);b?.classList.toggle('multi-selected',multiTabs.size>1&&multiTabs.has(t));b?.setAttribute('aria-selected',String(active===t||multiTabs.has(t)));}}
  root.classList.toggle('has-top-tabs',tabs.some(t=>t.topBar));
  layoutTopTabs();
  const shown=new Set();
 ...
 function layoutTopTabs(){const lane=$('.top-tabs'),nodes=[...lane.children];for(const item of topTabLayout(nodes.map(node=>({node,x:tabs.find(t=>t.id===node.dataset.tabId)?.topX||0,width:node.getBoundingClientRect().width})),lane.clientWidth,4,topTabAlignment)){item.node.style.left=item.left+'px';}}
 const topResize=new ResizeObserver(layoutTopTabs);topResize.observe($('.top-tabs'));ctx.effect(()=>topResize.disconnect());
 function select(t){if(!t)return;if(active!==t)active?.rememberPosition?.();if(docked&&dockMaximized){lastDock=docked;docked=null;}if(docked===t||(t.group&&docked?.group===t.group))docked=null;active=t;render(root.querySelectorAll('.tab').length===tabs.length);t.focus?.();}
 ...
 const api={isClosing:()=>closing,snapshotLayout:()=>{const encode=n=>n.children?{direction:n.direction,ratio:n.ratio,children:n.children.map(encode)}:{id:n.id};return {topTabPositions:Object.fromEntries(tabs.filter(t=>t.topBar).map(t=>[t.id,t.topX||0])),topTabs:tabs.filter(t=>t.topBar).map(t=>t.id),active:active?.id,docked:docked?.id,groups:[...new Set(tabs.map(t=>t.group).filter(Boolean))].map(g=>encode(g.tree)),sidebarId,sidebarHidden:$('.sidebar').classList.contains('collapsed'),sidebarWidth:$('.sidebar').style.width,dockHeight:$('.dock').style.height,dockMaximized};},restoreLayout:async state=>{for(const t of tabs){t.topBar=(state.topTabs||[]).includes(t.id);t.topX=state.topTabPositions?.[t.id]||0;}const decode=n=>n.children?{direction:n.direction,ratio:n.ratio,children:n.children.map(decode).filter(Boolean)}:tabs.find(t=>t.id===n.id);for(const tree of state.groups||[]){const group={tree:decode(tree),element:el('div','terminal-group')};for(const t of leaves(group.tree))t.group=group;drawGroup(group);}active=tabs.find(t=>t.id===state.active)||tabs[0]||null;docked=tabs.find(t=>t.id===state.docked)||null;dockMaximized=state.dockMaximized;$('.dock').style.height=state.dockHeight;$('.sidebar').style.width=state.sidebarWidth;await api.showPanel(state.sidebarId||'explorer');$('.sidebar').classList.toggle('collapsed',state.sidebarHidden);if(state.sidebarHidden)ctx.emit('sidebar.visibility',{id:sidebarId,visible:false});render();active?.focus?.();},rename,tabs,commands,root,$,render,remove,select,current,split,active:()=>active,close,closeAll,
  ...
  async showPanel(id,{preserve=false}={}){const p=panels.get(id);if(!p)return;ctx.emit('sidebar.leaving',{id:sidebarId,next:id});sidebarId=id;$('.sidebar').dataset.panel=id;$('.sidebar-heading span').textContent=id==='search'?p.label:p.label.toUpperCase();for(const [key,item]of panels)item.b.classList.toggle('selected',key===id);const previous=$('#sidebar-body'),body=replaceSidebarBody(previous,preserve);await p.renderPanel(body);if(preserve&&previous.isConnected&&sidebarId===id){const top=previous.scrollTop;previous.replaceWith(body);body.scrollTop=top;}},
 ...
 const toggleSecondary=()=>api.run('view.secondary');const secondaryButton=button('','显示/隐藏右侧栏',toggleSecondary,'secondary-toggle');secondaryButton.innerHTML='<svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2" fill="none" stroke="currentColor" stroke-width="1.5"/><path d="M16 4v16" stroke="currentColor" stroke-width="1.5"/></svg>';const corners=el('div','corner-controls');root.append(corners);const alignButton=button('≡','顶部标签排列',()=>{const r=alignButton.getBoundingClientRect();menuAt(r.right-190,r.bottom,[['free','自由任意（默认）'],['left','左对齐'],['right','右对齐'],['center','居中对齐']].map(([value,label])=>({label,checked:topTabAlignment===value,run:()=>{topTabAlignment=value;layoutTopTabs();layoutAPI('settings/layout/write',{topTabAlignment:value}).catch(logMessage);}})));},'top-tab-alignment');corners.append(alignButton,secondaryButton);
 ...
 window.harnessRequestQuit=async()=>{if(closing)return false;window.harnessQuitting=true;try{await api.flushFileOperations?.();const busy=[];for(const t of tabs.filter(t=>t.kind==='terminal')){const state=await t.checkClose?.();if(!state||state.busy)busy.push(t.title+(state?.reason?'：'+state.reason:''));}if(busy.length&&!window.confirm('以下终端仍有任务或无法确认状态：\n'+busy.join('\n')+'\n\n继续退出将结束这些终端任务。未保存文件会保留在会话备份中。是否继续？')){window.harnessQuitting=false;return false;}if(!api.session)throw Error('Session backup is not ready');await api.session.flush();return true;}catch(e){window.harnessQuitting=false;logMessage(e);window.alert('无法退出：'+(e.message||e)+'。应用已保留，请重试。');return false;}};ctx.effect(()=>delete window.harnessRequestQuit);
