...
export default {id:'bridge',async activate(ctx){
 ...
 await loadPreferences(ctx.get('api'));
 ctx.provide('terminal.connect',(resume)=>new WebSocket(`${location.origin.replace('http','ws')}/terminal?showIdentity=${localStorage.getItem('terminal-show-identity')==='on'?1:0}&token=${encodeURIComponent(token)}${resume?'&resume='+encodeURIComponent(resume):''}`));
}};
