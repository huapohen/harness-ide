...
export async function startHotUpdates(kernel){
 ...
 async function apply(next){if(busy||next.version===active.version||next.version===lastFailure)return;busy=true;
  try{
   if(next.js===active.js){if(next.css===active.css){active=next;await maintainCache(true);report('Up to date');return;}const html=await fetch('/__hot/'+next.version+'/index.html').then(r=>r.text());const doc=new DOMParser().parseFromString(html,'text/html'),hrefs=[...doc.querySelectorAll('link[rel=stylesheet]')].map(l=>l.getAttribute('href'));const old=[...document.querySelectorAll('link[rel=stylesheet]')],added=[];
    try{await Promise.all(hrefs.map(href=>new Promise((resolve,reject)=>{const link=document.createElement('link');link.rel='stylesheet';link.href=href;link.onload=resolve;link.onerror=()=>reject(Error('Stylesheet update failed'));added.push(link);document.head.append(link);})));await Promise.all(['codicon','seti'].map(font=>document.fonts.load('16px '+font)));}catch(e){added.forEach(l=>l.remove());throw e;}old.forEach(l=>l.remove());active=next;await maintainCache(true);report('Styles updated');return;
   }
