...
| 快捷键 | 功能 |
| --- | --- |
| Command+Shift+E / Command+B | 显示 / 隐藏左侧内容栏；点击最左边的功能图标可恢复内容栏 |
...
当前快捷键配置 [user/keybindings.json](user/keybindings.json) 已纳入 Git；`user/` 中的其他个人配置仍被忽略。新安装使用源码默认绑定；已有个人配置是完整数组，不会因源码增加默认绑定而自动合并。需要同步时可在设置中恢复默认（会替换个人绑定）。
